...
void *iterate(void *pres) {
  ...
  int score, score_r;
  score   = 1000000/vcycles; //to relieve relations with FREQ
  score_r = 1000000%vcycles; //to relieve relations with FREQ
  printf ("\nVCUNT_SIM: CoreMark 1.0 : %d (cycles)\n",vcycles); //CoreMark = iterations of a sec
  printf ("\nVCUNT_SIM: CoreMark 1.0 : %d, %d (iterations/sec)/MHz\n",score, score_r); //CoreMark = iterations of a sec
...
#if 0
	printf("\n###############################\n");
	printf(  "### CoreMark Starts Running ###\n");
	printf(  "###############################\n\n");
#endif

	// printf("SP: 0x%lx\n", (unsigned long)__builtin_frame_address(0));
	
	start_time = get_vtimer();
